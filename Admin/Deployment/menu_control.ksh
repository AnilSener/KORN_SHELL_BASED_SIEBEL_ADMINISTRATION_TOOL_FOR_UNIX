...
OUT=$(egrep 'level|id|name|menuheader' Deployment/$menufile|tr -d '\t'| sed 's/^<.*>\([^<].*\)<.*>$/\1/'|sed 's/ /_/g')
i=0;j=0
for tagvalue in ${OUT[@]} 
do
...
main_header=$(egrep 'mainheader' Deployment/$menufile|tr -d '\t'| sed 's/^<.*>\([^<].*\)<.*>$/\1/')
clear;
header_line "\n "$main_header
